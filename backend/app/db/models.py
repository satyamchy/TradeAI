"""
SQLAlchemy ORM models for the OneAI Stock Analyzer.

All persistent tables live in this single module so there is exactly one
declarative Base and one metadata registry for the whole app (previously
this was split across app/db/models.py and app/models/stock_models.py,
which used two separate Base() instances pointed at the same database file).
"""
...
class ActivePosition(Base):
    """A position opened by the AI agent that is being actively watched
    (e.g. by the watchdog service) for stop-loss / target / auto square-off."""
...
class EventLog(Base):
    """Structured agent/system event log, written by app.utils.logger.log_agent_event."""
...
# Public trading API tables.
from app.db.models_trading import Order, Trade, Position
